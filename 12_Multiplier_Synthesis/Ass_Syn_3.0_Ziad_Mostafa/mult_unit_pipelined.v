module mult_unit (
    ...
    // ------------------------------------------------------------
    // Stage 1 Accumulation (First 4 rows: pp[0]..pp[3])
    // ------------------------------------------------------------
    wire [15:0] level0;
    wire [15:0] level1;
    wire [15:0] level2;
    ...
    // ------------------------------------------------------------
    // Stage 2 Accumulation (Remaining 4 rows: pp[4]..pp[7])
    // ------------------------------------------------------------
    wire [15:0] level4;
    wire [15:0] level5;
    wire [15:0] level6;
    ...
    assign level4 = level3_pipe + (pp4_pipe << 4);  
    assign level5 = level4      + (pp5_pipe << 5); 
    assign level6 = level5      + (pp6_pipe << 6); 
    assign level7 = level6      + (pp7_pipe << 7); 
