...
module SYS_TOP_stress_tb;
    ...
    real current_bit_period = UART_CLK_PERIOD * PRESCALE;  // ~8680.54 ns
    ...
    initial begin
        ...
        $display("[TEST 4 DIAG] Before writes: SYS_CTRL state=%0d, RX_IN=%b, TX_OUT=%b, time=%0t",
                 u_dut.u_sys_ctrl.current_state, RX_IN, TX_OUT, $time);

        // Write 0x5A to Addr 0x06
        send_byte(8'hAA); send_byte(8'h06); send_byte(8'h5A);
        #(BIT_PERIOD * 3); // wait for SYS_CTRL to process write
        $display("[TEST 4 DIAG] After Write1: SYS_CTRL state=%0d, time=%0t",
                 u_dut.u_sys_ctrl.current_state, $time);

        // Write 0x3C to Addr 0x07
        send_byte(8'hAA); send_byte(8'h07); send_byte(8'h3C);
        #(BIT_PERIOD * 3); // wait for SYS_CTRL to process write
        $display("[TEST 4 DIAG] After Write2: SYS_CTRL state=%0d, time=%0t",
                 u_dut.u_sys_ctrl.current_state, $time);
        ...
        fork
            begin
                send_byte(8'hBB);
                send_byte(8'h06);
                $display("[TEST 4 DIAG] Read cmd sent. SYS_CTRL state=%0d, time=%0t",
                         u_dut.u_sys_ctrl.current_state, $time);
            end
            begin
        ...
        // Wait for TX to fully idle before starting ALU test (approx 15 bit periods)
        #(BIT_PERIOD * 15);
        ...
        ref_jitter  = 0.0;
        uart_jitter = 0.0;
        current_bit_period = UART_CLK_PERIOD * PRESCALE;
